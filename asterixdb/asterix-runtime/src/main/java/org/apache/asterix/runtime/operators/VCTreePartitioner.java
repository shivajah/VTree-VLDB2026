...
public class VCTreePartitioner {
     ...
     */
    public void partitionData(VCTreeStaticStructureCreatorOperatorDescriptor.DummyData dummyData, int K) throws HyracksDataException {
        ...
        int numPartitions = calculatePartitionsUsingShapiro(K, dummyData.tuples.size() * 1024L, frameSize, memoryBudget);
     ...
     */
    private void recursivelySeparateCentroids(int partitionId, List<Integer> centroids, int level) throws HyracksDataException {
        System.err.println("Level " + level + ": Processing " + centroids.size() + " centroids in partition " + partitionId);
        ...
        if (centroids.size() <= 1) {
            ...
            int subPartitions = calculatePartitionsUsingShapiro(centroids.size(), 
                centroids.size() * 1024L, frameSize, memoryBudget);
            ...
            for (Map.Entry<Integer, List<Integer>> entry : subPartitionToCentroids.entrySet()) {
                ...
                FileReference file = ctx.getJobletContext().createManagedWorkspaceFile(
                    "VCTreeSubPartition_" + partitionId + "_" + subPartitionId + "_level_" + level + "_" + System.currentTimeMillis());
     ...
     */
    private void separateAndWriteToFiles(int partitionId, List<Integer> centroids, int level) throws HyracksDataException {
        ...
        Map<Integer, List<VCTreeStaticStructureCreatorOperatorDescriptor.DummyTuple>> centroidGroups = groupByCentroidId(allData);
        
        // Create individual file for each centroid
        for (int centroidId : centroids) {
            List<VCTreeStaticStructureCreatorOperatorDescriptor.DummyTuple> centroidData = centroidGroups.get(centroidId);
     ...
     * Read data from partition file.
     */
    private List<VCTreeStaticStructureCreatorOperatorDescriptor.DummyTuple> readPartitionData(int partitionId) throws HyracksDataException {
    ...
    private void writeCentroidDataToPartition(int partitionId, List<Integer> centroids) throws HyracksDataException {
        ...
        if (writer != null) {
            for (int centroidId : centroids) {
                ...
                VCTreeStaticStructureCreatorOperatorDescriptor.DummyTuple tuple = new VCTreeStaticStructureCreatorOperatorDescriptor.DummyTuple();
    ...
    private void createInitialPartitionFiles(int numPartitions) throws HyracksDataException {
        for (int i = 0; i < numPartitions; i++) {
            FileReference file = ctx.getJobletContext().createManagedWorkspaceFile(
                "VCTreePartition_" + i + "_" + System.currentTimeMillis());
            RunFileWriter writer = new RunFileWriter(file, ctx.getIoManager());
            writer.open();
     ...
     */
    private void writeDataToPartitions(VCTreeStaticStructureCreatorOperatorDescriptor.DummyData dummyData) throws HyracksDataException {
