...
public abstract class OptimizedSearchTestDriver {
    ...
    public void optimizedSearchThreeDimension() throws Exception {
        ...
        // Query configuration for 3D test:
        // Query at [5, 0, 0], D(q, C) = 5.0, expect top-5 closest records
        double[] queryVector = { 5.0, 0.0, 0.0 };
        int queryK = 5;
        List<String> expectedPKs = Arrays.asList("pk_opt_5", "pk_opt_4", "pk_opt_6", "pk_opt_3", "pk_opt_7");

        runTest(centroidSerdes, dataRecordSerdes, centroids, numClustersPerLevel, centroidsPerCluster, 3, dataRecords,
                queryVector, queryK, expectedPKs);
    }
    ...
    public void twoDimensionTwoLevels() throws Exception {
        ...
        List<List<Integer>> centroidsPerCluster = Arrays.asList(
                Arrays.asList(4),           // Level 0: 1 cluster with 4 centroids
        ...
        // Query configuration for 2D test:
        // Query at [30, 30] is closest to centroid c4 at [25, 25]
        // D(q, c4) = sqrt((30-25)^2 + (30-25)^2) = sqrt(50) ≈ 7.07
        //
        // Expected top-5: records from c4 in +x and +y directions (towards query)
        // Records closer to [30,30] have higher distance from centroid [25,25]
        // pk_2d_c4_16: [26,25], d=1.0, D(q,x)=√41≈6.40
        // pk_2d_c4_18: [25,26], d=1.0, D(q,x)=√41≈6.40
        // pk_2d_c4_12: [25.8,25], d=0.8, D(q,x)=√42.64≈6.53
        // pk_2d_c4_14: [25,25.8], d=0.8, D(q,x)=√42.64≈6.53
        // pk_2d_c4_8: [25.6,25], d=0.6, D(q,x)=√44.36≈6.66
        double[] queryVector = { 30.0, 30.0 };
        int queryK = 5;
        List<String> expectedPKs = Arrays.asList(
                "pk_2d_c4_16", "pk_2d_c4_18", "pk_2d_c4_12", "pk_2d_c4_14", "pk_2d_c4_8");
    ...
    /**
     * Get 2D leaf centroids for insert tests.
     * Returns the static LEAF_CENTROIDS_2D array.
     */
    protected static double[][] getLeafCentroids2D() {
        return LEAF_CENTROIDS_2D;
    }

    /**
     * Generate insert records for 2D two-layer dataset.
     * Insert tuple format: <vector, primary_key>
     *
     * Generates recordsPerCluster records for each of the 16 leaf centroids.
     * Records are placed in rings around each centroid, similar to bulk load records
     * but with a different primary key prefix ("pk_ins_") for identification.
     *
     * @param recordsPerCluster Number of records to generate per leaf cluster
     * @return List of lists, where each inner list contains tuples for one cluster
     */
    protected List<List<ITupleReference>> generate2DInsertRecords(int recordsPerCluster) throws Exception {
        List<List<ITupleReference>> allRecords = new ArrayList<>();

        // Generate records for each of the 16 leaf centroids (c4-c19)
        for (int centroidIndex = 0; centroidIndex < LEAF_CENTROIDS_2D.length; centroidIndex++) {
            List<ITupleReference> clusterRecords = new ArrayList<>();
            ...
            double baseDistance = 0.15; // Slightly different offset from bulk load (0.2)
            int recordCount = 0;
