...
/**
 * LSMVCTree pruning search test.
 * Tests the LSMVCTreeBlockedCursor (optimized/pruning search) with triangle inequality pruning.
 *
 ...
 */
public class LSMVCTreePruneSearchTest extends QuantizedSearchTestDriver {
    ...
    protected void runTest(ISerializerDeserializer[] centroidSerdes, ISerializerDeserializer[] dataRecordSerdes,
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("LSMVCTree Pruning Search Test: {} levels, {} centroids, {} leaf clusters, {}D vectors",
        ...
        try {
            ...
            // 4. Validate: pruning search for each query case
            for (int i = 0; i < queryCases.size(); i++) {
                ...
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info("Query case {}/{} (pruning) succeeded: K={}", i + 1, queryCases.size(), qc.queryK);
                }
            }
